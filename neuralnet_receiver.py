...
if __name__ == "__main__":
    ...
    np.save("receiver_models/learning_rates.npy", np.fromiter(lr_dict.values(), dtype=float))
    ...
    for M in PAM:
        ...
        model_dict[M], train_loss[M], test_loss[M] = train_decision_nn(model, M, 
                                                                       ...
                                                                       epochs=15*M,
                                                                       earlystop=False)
        np.save(f'receiver_models/loss/train_loss_{M}.npy', train_loss[M])
        np.save(f'receiver_models/loss/test_loss_{M}.npy', test_loss[M])
        torch.save(model_dict[M], f"receiver_models/model_{M}pam.pth")
    ...
    for M in PAM:
        ...
        np.save(f"receiver_models/SER/Pe_{M}.npy", Pe)
        np.save(f"receiver_models/SER/SER_{M}.npy", err)
    

    import plot_receiver
